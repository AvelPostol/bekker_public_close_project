        ...
        foreach($p['deals'] as $deal){
            if($this->reauireField($deal)){
                ...
                if($this->GetEntityCallType($deal)){
                    ...
                    // timetocall
                    $data[$deal['ID']]['uf_time_call'] = $this->TimeUFCall($deal['UF_CRM_1693572313224']);

                    // ID DEAL 
                    $data[$deal['ID']]['id_crm_deal'] = $this->IDDeal($deal['ID']);

                    // ТИП ЗВОНКА
                    $data[$deal['ID']]['type_call'] = $this->GetEntityCallType($deal);

                    // контакт сделки
                    $data[$deal['ID']]['contact_id'] = $this->ContactID($deal['CONTACT_ID']);

                    // ответственный оператор
                    $data[$deal['ID']]['id_responsible'] = $this->ResponsibleID($deal['ASSIGNED_BY_ID']);

                    // перерыв
                    $data[$deal['ID']]['breaktime'] = $this->WorkManager_Breaktime($deal['ASSIGNED_BY_ID']);

                    // состояние рабочего дня 
                    $data[$deal['ID']]['start_day'] = $this->WorkManager_start_day($deal['ASSIGNED_BY_ID']);
