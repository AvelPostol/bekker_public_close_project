...
class ChatManager_test {
  ...
  public function __construct($metod) {
    ...
    $oneWeekAgo = new \DateTime('-720 hour'); //  $oneWeekAgo = new \DateTime('-1 hour');
  ...
  public function getClient($p){
    ...
    if(!$BitrixCommon['CONTACT_ID'] || $BitrixCommon['CONTACT_ID'] == 0 || !isset($BitrixCommon['CONTACT_ID'])){
      ...
      if($ContactGet == 'update'){
        ...
        $contact = $this->BitrixCommon->ContactUpdate(['ID' => $BitrixCommon['CONTACT_ID'], 'updateField' => $client]);
  ...
  public function getFileBazis($p){
    foreach($p['files'] as $keyFl => $file) {
      ...
      $chatManager = new ChatManager_test($metod);
  ...
  public function getFiles($p){
    ...
    if($IblokItems){
      ...
      if(isset($IblokItemCheckBazis['orderIsChecked'])){
        ...
        if(isset($CheckUpdate['err']) && !empty($CheckUpdate['err'])){
          $Base = new \BazisCM\Workspace\Tools\Base();
          $error = "не получилось обновить документ: ".$p['data']['number'].' '.$CheckUpdate['err'];
          $Base->GoWide($error);
        }
      ...
      if(isset($CheckAdd['err']) && !empty($CheckAdd['err'])){
        $Base = new \BazisCM\Workspace\Tools\Base();
        $error = "не получилось сгенерировать документ: ".$p['data']['number'].' '.$CheckAdd['err'];
        $Base->GoWide($error);
      }
    }
